...
public class VulkanRenderer
    ...
    // Refactor Size stretches the guest frame across the complete physical surface.
    private volatile boolean refactorSizeMode = false;
    ...
    private void buildAndSubmitFrame() {
        ...
        if (refactorSizeMode) {
            // The guest/X-server can be shorter than the Android surface. Refactor Size
            // intentionally uses the whole physical surface so no lower frame is left.
            final float sx = xServer.screenInfo.width > 0
                    ? (float) surfaceWidth / (float) xServer.screenInfo.width : 1f;
            final float sy = xServer.screenInfo.height > 0
                    ? (float) surfaceHeight / (float) xServer.screenInfo.height : 1f;
            XForm.makeTransform(sceneXform, 0f, 0f, sx, sy, 0);
            useScissor = true;
        ...
        if (fullscreen || refactorSizeMode) {
            viewX = 0;
            viewY = 0;
            viewW = surfaceWidth;
            viewH = surfaceHeight;
    ...
    /** Keep Refactor Size rendering and input on the exact same full-surface rectangle. */
    public void setRefactorSizeMode(boolean enabled) {
        if (refactorSizeMode == enabled) return;
        refactorSizeMode = enabled;
        viewportNeedsUpdate = true;
        requestRenderCoalesced();
    }

    public boolean isRefactorSizeMode() { return refactorSizeMode; }
