...
class TouchpadView(
    ...
    private fun updateXform(outerWidth: Int, outerHeight: Int, innerWidth: Int, innerHeight: Int) {
        ...
        // Refactor Size renders the guest across the complete physical surface. Use the
        // exact inverse of that full-surface transform so touch and pixels share one space.
        if (renderer.isRefactorSizeMode) {
            XForm.set(
                xform,
                sceneW.toFloat() / surfaceW.toFloat(),
                0f,
                0f,
                sceneH.toFloat() / surfaceH.toFloat(),
                0f,
                0f,
            )
            return
        ...
        // Do not bypass ViewTransformation in fullscreen. The resize/fit feature changes
        // the rendered frame geometry even when the Android surface itself is fullscreen.
        // Using a second fullscreen-only scale here would reintroduce the exact image/touch
        // mismatch this transform is meant to prevent.
        ...
        if (vt.mode == ViewTransformation.FILL_MODE_STRETCH) {
             ...
             *
             * FIT now has viewOffsetX/Y == 0, so no padding is created and the touch map
             * follows the rendered pixels one-to-one. ZOOM keeps the same math for its crop.
             */
    ...
    private fun getTouchscreenPointerInside(event: MotionEvent): Boolean {
        ...
        if (renderer.isRefactorSizeMode) return true
        val vt = renderer.viewTransformation
