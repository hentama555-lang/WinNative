...
class TouchpadView(
    ...
    private fun updateXform(outerWidth: Int, outerHeight: Int, innerWidth: Int, innerHeight: Int) {
        ...
        // NOTE: renderer.isFullscreen() is the renderer's own Fullscreen-Stretched toggle
        // (VulkanRenderer#toggleFullscreen / main_menu_toggle_fullscreen) - not the Android
        // surface/system-UI fullscreen state. When it's on, buildAndSubmitFrame() sets the
        // GL/Vulkan viewport to the full physical surface (0,0,surfaceW,surfaceH) with an
        // identity sceneXform, bypassing ViewTransformation's FIT/ZOOM letterbox entirely -
        // the guest screen is stretched edge-to-edge in that state regardless of the selected
        // resize mode. The touch map must mirror that exactly (no viewOffset, plain surface/
        // scene ratio), or taps drift from the rendered pixels - worst near the edges -
        // whenever FIT/ZOOM is the selected resize mode.
